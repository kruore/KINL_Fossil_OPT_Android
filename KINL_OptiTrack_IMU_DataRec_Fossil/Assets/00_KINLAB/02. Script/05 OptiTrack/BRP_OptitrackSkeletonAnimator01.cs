...
namespace _3DCL
{
    ...
    /// </remarks>
    public class BRP_OptitrackSkeletonAnimator01 : MonoBehaviour
    {
        ...
        public string SkeletonAssetName = "Skeleton 01";
        ...
        void FixedUpdate()
        {
            ...
            if (skelState != null)
            {
                ...
                for (int i = 0; i < m_skeletonDef.Bones.Count; ++i)
                {
                    ...
                    OptitrackPose joint; ///SangBin : ReNaming : bonepose - >joint
                    GameObject boneObject;
                    bool foundPose = skelState.BonePoses.TryGetValue(boneId, out joint);
                    bool foundObject = m_boneObjectMap.TryGetValue(boneId, out boneObject); ///SangBin : Memo : pointer
                    if (foundPose && foundObject)
                    {
                        boneObject.transform.localPosition = joint.Position;
                        boneObject.transform.localRotation = joint.Orientation;
                        //Debug.Log(i.ToString() + " : " + boneObject.name.ToString());

                        //Local Skeleton
                        GetBodyJointData(ref sb, joint.Position, joint.Orientation);


                        //Grobal Skeleton
                        //GetBodyJointData(ref sb, boneObject.transform.position, boneObject.transform.rotation);
                    }
                }
                //반복문 나누면 되겟네 하나는 데이터값 컨테이너에 저장 && 하나는 이제 각 아바타 조인트에 데이터 업데이트
                //그 다음에 특정 조인트 아디 값만 잘 알면 그 아디값들 데이터랑 포즈데이터랑 비교해서 체크하면 되겟군 
                ...
                //잠시 꺼둠 0323 프레임 저하 나중에 바꾸자
                //DataRecorder.instance.WriteData(sb.ToString(), currentTime);
                //DataRecordingComponent.instance.Enequeue_Data(sb.ToString());
                
                _KINLAB.GM_DataRecorder.instance.Record_Data_IMU_OptiTrackSkel(sb.ToString());
            }
        }
        ...
        private void GetBodyJointData(ref  StringBuilder sb, Vector3 position, Quaternion orientation)
        {
            sb.AppendFormat("{0:F4}", position.x).Append(',');
            sb.AppendFormat("{0:F4}", position.y).Append(',');
            sb.AppendFormat("{0:F4}", position.z).Append(',');

            sb.AppendFormat("{0:F4}", orientation.w).Append(',');
            sb.AppendFormat("{0:F4}", orientation.x).Append(',');
            sb.AppendFormat("{0:F4}", orientation.y).Append(',');
            sb.AppendFormat("{0:F4}", orientation.z).Append(',');

            Vector3 orientation_Euler = orientation.eulerAngles;

            sb.AppendFormat("{0:F4}", orientation_Euler.x).Append(',');
            sb.AppendFormat("{0:F4}", orientation_Euler.y).Append(',');
            sb.AppendFormat("{0:F4}", orientation_Euler.z).Append(',');
        }
        ...
        private void CacheBoneNameMap(OptitrackBoneNameConvention convention, string assetName)
        {
            ...
            switch (convention)
            {
                case OptitrackBoneNameConvention.Motive:
                    ...
                    m_cachedMecanimBoneNameMap.Add("Head", assetName + "_Head"); //5
                    ...
                    m_cachedMecanimBoneNameMap.Add("RightToeBase", assetName + "_RToe"); //21
                    ...
                    m_cachedMecanimBoneNameMap.Add("Right Little Distal", assetName + "_RPinky3"); //51
                    break;
