...
public class OptitrackStreamingClient : MonoBehaviour
{
    ...
    public Dictionary<Int32, OptitrackMarkerState> m_latestMarkerStates = new Dictionary<Int32, OptitrackMarkerState>();
    ...
    public List<Vector3> m_RigidbodyMarkerPos = new List<Vector3>();
    ...
    private void OnNatNetFrameReceived( object sender, NatNetClient.NativeFrameReceivedEventArgs eventArgs )
    {
        ...
        try
        {
            ...
            for (int rbIdx = 0; rbIdx < frameRbCount; ++rbIdx)
            {
                ...
                ///NatNet_Frame_RigidBody_Marker_GetPosition
                //Int32 marker_count_INT = 0;
                //marker_count_INT = GetRigidBodyDefinitionById(rbData.Id).Markers.Count;
                ////Debug.LogError(marker_count_INT);

                //m_RigidbodyMarkerPos.Clear();
                //for (int i = 0; i < marker_count_INT; i++)
                //{
                //    Vector3 tempV = Vector3.zero;
                //    result = NaturalPoint.NatNetLib.NativeMethods.NatNet_Frame_RigidBody_Marker_GetPosition(pFrame, rbIdx, i, out tempV.x, out tempV.y, out tempV.z);
                //    NatNetException.ThrowIfNotOK(result, "NatNet_Frame_RigidBody_Marker_GetPosition failed.");

                //    m_RigidbodyMarkerPos.Add(tempV);
                //}
            ...
            for (int skelIdx = 0; skelIdx < frameSkeletonCount; ++skelIdx)
            {
                ...
                // Enumerate this skeleton's bone rigid bodies.zz
                Int32 skelRbCount;
            ...
            //Debug.Log("AAA Market Count " + MarkerCount);

            ///*            string str = string.Empty;
            //            bool tempbb = false;
            //            if(tempCount != MarkerCount)
            //            {
            //                tempCount = MarkerCount;
            //                tempbb = true;
            //            }*/


            ///// for raw Marker Recorder
            ...
            //    //m_RigidbodyMarkerPos.Add(markerPos);

            //    //str += m_latestMarkerStates[marker.Id].Id.ToString() + "#";

            //    //OptitrackMarkerState aa = new OptitrackMarkerState();
            //    //m_latestMarkerStates.TryGetValue(marker.Id, out aa);
            //    //Debug.Log(aa.Id + "  " + aa.Position.x);

            //    _KINLAB.GM_DataRecorder.instance.EnDic_MarkerDic(marker.Id, markerPos);
            //}

            ///*            if (tempbb)
            //            {
            //                string tempstr = str + "#MarkerCount:" + MarkerCount;
            //                _KINLAB.GM_DataRecorder.instance.Debug_MarkerMisfire(tempstr);
            //                //Debug.Log(str + "#MarkerCount:" + MarkerCount);
            //            }*/


            //_KINLAB.GM_DataRecorder.instance.Check_MarkerDataState();
